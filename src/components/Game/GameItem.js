...
function GameItem(props) {
    ...
    return (
        <div className="flex relative mx-auto max-w-7xl border-2 border-red-400 ">
            ...
            <div className="border-4 border-lime-500 w-full flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4">
                <div className="flex flex-row md:flex-none">
                    <img src={props.image} alt="Game" className="w-44 md:w-60 h-72 md:h-80 -mt-24 ml-2 md:ml-0 md:-mt-40 z-10 max-w-full" />
                    
                    <div className="flex flex-col space-y-10 md:hidden ml-2">
                        <h2 className="mt-2 text-xl font-bold">{props.gameDetails.name}</h2>
                        <div className="border border-green-300">
                            <h2 className="text-lg mb-2 opacity-60">{props.gameDetails.releaseDate}</h2>
                            <h2 className="text-lg opacity-60">{props.gameDetails.developer}</h2>
                        </div>

                    </div>
                </div>
                <div className="border-2 border-orange-500 flex flex-col w-full space-y-2 blg:flex-row blg:space-y-0">
                    <GameTabs  genres={props.gameDetails.genresNames}
